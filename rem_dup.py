...
keys_to_include_in_hash = ["title", "link", "keyword"]
...
def scroll_over_all_docs():
    data = es.search(index="adqlemma", scroll='1m',  body={"query": {"match_all": {}}})
    # Get the scroll ID
    sid = data['_scroll_id']
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
      if len(array_of_ids) > 1:
        ...
        matching_docs = es.mget(index="adqlemma", body={"ids": array_of_ids})
        for doc in matching_docs['docs']:
            ...
            #print("doc=%s\n" % doc)
            print(array_of_ids)
            for index,ids in enumerate(array_of_ids):
                if 1 == index:
                    continue
                es.delete(index="adqlemma", id=ids, ignore=404)
                #print("deleted id = %s" %ids)
def main():
